...
### `GET /api/users/stats/summary` — 플랫폼 전체 통계 요약
...
{
  "total_users": 1024,
  "today_plogging_users": 38,
  "total_sessions": 5200,
  "total_distance_meters": 98000000,
  "total_trash_count": 412000
}
```
...
| `today_plogging_users` | 오늘(KST 기준) 플로깅한 고유 사용자 수 |
| `total_sessions` | 플랫폼 전체 완료된 세션 수 |
| `total_distance_meters` | 플랫폼 전체 누적 이동 거리 (미터) |
| `total_trash_count` | 플랫폼 전체 누적 쓰레기 수거 수 |

> **오늘 플로깅한 사람 수 정의**: `started_at`이 오늘(KST 00:00~23:59)이고 상태가 `completed`, `active`, `paused` 중 하나인 세션의 고유 사용자 수. `expired` 세션은 제외됩니다.

---

### `GET /api/users/stats/leaderboard` — 사용자 통계 리더보드
...
| 파라미터 | 타입 | 기본값 | 설명 |
|----------|------|--------|------|
| `sort_by` | string | `distance` | 정렬 기준: `distance` \| `sessions` \| `duration` |
| `limit` | int | `20` | 조회 수 (1~100) |
| `offset` | int | `0` | 건너뛸 수 |
...
|------|------|
| `items` | 정렬된 사용자 통계 목록 |
| `total` | 완료된 세션이 1개 이상인 전체 사용자 수 |

> `completed` 상태 세션만 집계합니다.

---

### `GET /api/users/{user_id}/stats` — 특정 사용자 누적 통계
...
| **사용자** |
| `GET` | `/api/users/me/stats` | O | 내 누적 통계 조회 |
| `GET` | `/api/users/stats/summary` | O | 플랫폼 전체 통계 요약 |
| `GET` | `/api/users/stats/leaderboard` | O | 사용자 통계 리더보드 |
| `GET` | `/api/users/{user_id}/stats` | O | 특정 사용자 누적 통계 조회 |
